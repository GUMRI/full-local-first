...
export function list<T extends object, U = any>(
    ...
    for (const fileInput of files) {
      ...
      try {
        ...
        const storedFile: StoredFile = {
          ...
          lastModified: fileInput.data.lastModified,
  ...
  function _matchesFilter(item: Item<T, U>, where: FilterArgs<T>['where']): boolean {
    ...
    for (const key of Object.keys(where) as (keyof T & keyof FilterArgs<T>['where'])[]) {
      ...
      if (typeof condition === 'object' && condition !== null && !Array.isArray(condition)) {
        ...
        if (condition.lt !== undefined && !(itemValue < condition.lt)) return false;
        if (condition.lte !== undefined && !(itemValue <= condition.lte)) return false;
        if (condition.gt !== undefined && !(itemValue > condition.gt)) return false;
        if (condition.gte !== undefined && !(itemValue >= condition.gte)) return false;
  ...
  const upsert = async (input: CreateItemInput<T, U> | UpdateItemInput<T, U>): Promise<Item<T, U>> => {
    ...
    if ('id' in input && input.id) {
      ...
      if (existingItem) {
        return update(input as UpdateItemInput<T, U>);
      } else {
        ...
        return create({ 
            ...
            createdBy: createData.updatedBy, // Use updatedBy as createdBy for consistency
  ...
  const listRef: ListRef<T, U> = {
    ...
    replicationStatus$: replicationEngine ? replicationEngine.isReplicating.asReadonly() : signal(false).asReadonly(),
